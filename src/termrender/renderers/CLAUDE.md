...
**`mermaid_text.py`** — `decode_entities(text)` and `apply_emphasis(text)`, the two shared label markup steps.

`apply_emphasis` turns mermaid's supported inline emphasis tags (`<b>`/`<strong>`, `<i>`/`<em>`) into ANSI SGR runs; **nothing else is treated as markup** — a `<span>` stays literal, because inventing support for arbitrary HTML is not this renderer's job. It runs *before* `decode_entities`, never after: an author who wrote `&lt;b&gt;` escaped the tag deliberately and wants the characters. The escapes then ride through layout untouched because every geometry decision measures with `visual_len` — `<b>dialect</b>` is 14 source characters and 7 visible ones, so a width pass that measured the styled string would widen every box holding an emphasized label. `mermaid.py`'s `render` drops the escapes for `--color off`, which is why the width pass must stay color-blind: layout runs once, styled, and the two color modes differ only in escapes.

The primitives that make that safe live in `style.py`: `styled_clusters(s)` walks a string as `(active-SGR, visible-cluster)` pairs so an escape is never taken for a glyph nor split across two cells, and `active_sgr(s)` folds a string down to the styling it leaves running — what `mermaid_flow_layout.py::_carry_sgr` re-opens on each wrapped line. **Reach for those two, not `grapheme_clusters`, anywhere a label is laid out character by character.**

**When you end a style run that sits mid-line, close it with `style.py::sgr_transition(open, wanted)`, never a bare `RESET`** — SGR 0 also clears the colour the *host* embedding this output set, so every cell after an emphasized word (a label's padding, the box's right border) loses it while the rest of the diagram keeps it. `sgr_transition` emits the attribute's own disable code (22 for bold, 23 for italic) and falls back to a reset only for an attribute that has none. `style()`'s whole-run close goes through it too; a run carrying a colour still resets, because no code restores a colour this process never set.

`decode_entities(text)` is the shared label decode step. Mermaid labels are HTML, so authors escape grammar-colliding characters as `&lt;`/`&#35;` or mermaid's hash form `#quot;`/`#35;`; every native renderer calls `decode_entities` where it normalizes a label (next to its `<br/>` handling), *after* parsing has used the raw text for structure. Decode at the label, never over the source: `--&gt;` inside a quoted label must not become a real connector and `&amp;` must not become flowchart's `&` node-group separator. Decoding is stricter than `html.unescape` — a code needs its closing `;`, because the stdlib decodes `&params` to `¶ms`.
...
A text pie chart is worse than a labeled bar chart at the same information density (per the ratified plan), so `parse_pie` extracts `(title, items)` from the `pie [showData]` / `title …` / `"label" : value` grammar and `render` hands the items to `charts.py::render_bar` via a synthetic `Block(type=BlockType.BAR, ...)` — no duplicated bar-drawing logic. Each item's `unit` attr is set to the pre-formatted percentage string (e.g. `" (40.0%)"`) so `render_bar`'s existing value-formatting appends it for free. Called with `color=False` always — a mermaid diagram's own drawing is monochrome by design; the only styling anywhere in mermaid output is the emphasis a node/edge label's `<b>`/`<i>` markup asked for, and because escapes cost no display columns `--color on` and `--color off` are guaranteed to lay out identically. Unparseable/empty input degrades to the raw source lines, same degradation contract every other mermaid renderer in this package upholds.
